...
Since the objective of the system is to maintain stable hovering in front of the window, the performances of the system is measured by the amount of deviation from the center line of the window in the critical axis $x$, i.e., we want to minimize $\left| x \right|$ (see Figure~\ref{fig:axis}). Our goal is to achieve maximum performance with minimal amount of processing time. Obviously, both goals can not be achieved together. We aim at achieving a good trade-off between processing resources and quality of measurement. The focus is on the main consumer of computation resources: the image processing algorithm.
We propose to use a combination of modes in order to improve the resource utilization. Specifically, we aim at taking resources (high CPU time) only when needed. By this we will reduce the average CPU usage without significantly affecting the performance.
We do this by dynamically switching between camera resolutions during the flight, i.e., the observer modes correspond to image resolutions (see Section~\ref{sec:Observer Design}).
...
On the other hand, the low quality mode provides mean error of 30\textit{cm} (three times the error obtained in the high quality mode) using only 2.1\% CPU usage~\footnote{The CPU usage percentage is the average usage, and is calculates as ${\text{total time spend in image processing}}/{\text{total flight time}}$. }.
We explain next how we defined an \textit{reactive scheduling specification} that combines the advantages of both modes.

%displacement ($x_{k|k}$) schedulers
We first examined a straight-forward solution based directly on the current position (the $x$ axis of $\hat{x}_{k|k}$). Using the simple guarded automaton $A_{x}$ presented in Figure~\ref{fig:test_automata}, we defined a dynamic observer that chooses the operation mode (high or low quality) based on the absolute value of the $x$ part of $\hat{x}_{k|k}$. If the drone is closer to the center line of the window, denoted by $|x| < T_x$, we consider it as a ``safe'' state and go to $L$ mode that schedules the low quality image.
Similarly, ``BAD'' states is considered when the drone is far, denoted by $|x| > T_x$. In this case, we use high quality image in order to ``fix'' the state.
The threshold $T_x$ can be changed to define the desired performances and processing time trade-off.
See Section~\ref{sec:results} for the data we collected by experiments to validate that this indeed achieves the desired goals. 
...
From the Low quality experiment graph, shown in Figure~\ref{fig:testPlot}, we can see that $\tilde{y}_{k|k}$ accumulates proportional to the deviation from the center of the window (in the $x$ axis). This means that we can use $\tilde{y}_{k|k}$ values to predict high deviation from $x$.

In the second experiments set we used the $A_{err}$ automaton presented in Figure~\ref{fig:test_automata}. This automaton is similar to $A_x$ with the difference that it switches states based on $\tilde{y}_{k|k}$ value.
In this case the observer operates in low quality when $\tilde{y}_{k|k} < T_{l}$ (``GOOD'' state) and switches to high quality when $\tilde{y}_{k|k} > T_{h}$ (``BAD'' state).
The thresholds $T_{l}$ and $T_{h}$ were initially taken from the low and the high quality graphs and the were fine tuned by experiments. 
Different $T_{l}$ and $T_{h}$ give different trade-offs of performances and processing time.
In Section~\ref{sec:results} we show that this dynamic observer gives almost the same performance as the high quality observer with a reduction of a factor of $\frac{1}{2}$ in the processing time.
...
In Figure~\ref{fig:test_automata} there is two examples of such combinations. $A_{comp1}$ tries to save unnecessary CPU usage while the drone is in ``safe'' state, close to the center, by using only low quality, and activates $A_{err}$ only if the drone goes far from the center ($T_{x}$cm from the origin).
$A_{comp2}$ adds another constraint: if the drone goes even farther away ($T_{x2}$cm from the origin) use only high resolution to bring it back to safety.
The results show even better resource utilization.
%TODO - hodai: why not resource utilization?
%The results show even better trade-offs with these automata.
...
We show that by applying reactive scheduling methods we can control the \textit{desire} ``trade-off'' between performance and CPU usage, but the main goal is to chive the best ``resources utilization rate'', this mean using \textbf{less CPU for the same level of performance}.
...
%TODO - reactive with wind - need to extract info from matlab
Another advantage we achieved from using the dynamic schedulers is a different type of reactiveness, the schedule profile adapts itself to changes in environmental conditions, in order to maintain the performance.
We experiment this ability by introducing the flying drone to wind. 
We create artificial wind by turn on a fan while the drone is flying using the $A_{err}$ automaton and the thresholds values $T_l=10$ , $T_h=15$ (number 7 in Table~\ref{tab:results}).
In this experiment the mean displacement ($mean(|x|$) was 11.8cm which is very close to the displacement without the wind, and the average CPU usage was 13.2\%, instead of 11.7\% usage without the wind.
This experiment best demonstrate our statement, that resources is allocated only when is needed.
