...
The image processing algorithms cn run in different operation modes, each mode with different accuracy (see Section~\ref{sec:Analysis}).
The constant $K$ represents the ratio between the variance of the process noise and the variance of the measurement noise. To achieve best peformace, we define a separate value for $K$ for each mode.
...
We explain next how we defined an \textit{adaptive scheduling specification} that combines the advantages of both modes.
...
From the Low quality experiment graph we can see that $\tilde{y}_{k|k}$ accumulates proportional to the deviation from the center of the $x$ axis. This means that we can use $\tilde{y}_{k|k}$ values to predict high deviation from $x$.

In the second experiments set we used the $A_{err}$ automaton presented in Figure~\ref{fig:test_automata}. This automaton is similar to $A_x$ wit the difference that it switches states based on $\tilde{y}_{k|k}$ value.
...
$A_{comp2}$ adds another constraint: if the drone goes even farther away ($T_{x2}$cm from the origin) use only high resolution to bring it back to safety. The results show even better trade-offs with these automata.
...
\begin{figure}[htbp]
    ...
    \caption{The main Automata we used for the specifications in the experiments. 
...
We evaluate the result by the \textit{right} ``trade-off'' between performance and CPU usage, but the main goal is to chive the best ``resources utilization rate'', this mean using \textbf{less CPU for the same level of performance}.
...
In order to evaluate the performance we need to know the acurate position of the drone during the flight, we use a system called \textit{OptiTrack}.
