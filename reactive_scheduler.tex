...
The $A_{err}$ automaton (shown in Figure~\ref{fig:test_automata}), that is based on $\tilde{y}_{k|k}$, appears to be even more effective.
...
Testing the more complex schedulers can contribute even more to the performance and utilization, scheduler $A_{comp1}$ for example achieve a bit better resource utilization.
...
Specifically, we created artificial wind by turning  on a fan while the drone was flying. The scheduler was the $A_{err}$ automaton with the thresholds values $T_l=10$ , $T_h=15$ (number~7 in Table~\ref{tab:results}).
Figure~\ref{fig:windPlot} shows an experiment flight with the CPU usage along the flight where the first part of the flight, colored in blue, is indoor flight without any external wind, and during the rest of the flight we can see increasing CPU usage, colored in red, that is the scheduler respond to the wind disturbance.
...
This experiment further demonstrates our idea: resources are  allocated only when needed.
...
\begin{figure}[htbp]
    ...
    \caption{This figure demonstrates how the reactive scheduler adapt to the environment changes, in particular, to change of the wind. The upper part shows the average CPU usage during the flight, and the bottom is the true position at the same time. The first part of the flight (the left Blue graph) is without any external wind and in the rest of the flight (right red graph)  the drone was expose to artificial wind. See that the flight exposed to wind use more CPU to overcome the wind disturbance. }
    \label{fig:windPlot}
\end{figure}
...
We introduced a new approach of resource allocation that allows dynamic scheduling depending on the current state and needs of the system.
We present reach interface between tasks and scheduler based on automata theory, that allows to define complex task requirements, and advice how to find those requirements.
We also prove the contribution of dynamic scheduling in terms of control performance and resource utilization by computer simulation and with real practical case-study.

The use of automata allows to compose and analyze the scheduling characteristics, we plane to base on automata theory and hybrid automata theory to develop new techniques and tools for automatically generate task specification automata to meet the control requirement characteristics.
