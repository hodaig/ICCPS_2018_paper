...
Pitch and roll controllers composed from, low level attitude controller which his input is the required pitch or roll angel accordingly, and high level position controller which his input is the required distance or $x$ position relative to the window accordingly, the high level controller outputs the required angle (acceleration) to the low level controller as show in Figure~\ref{fig:controllerStracture}.
...
In Figure~\ref{fig:test_automata} there is two examples of such combination, $A_{comp1}$ try to save unnecessary CPU usage while the drone is in ``safe'' state, close to the center, and activate $A_{err}$ only if the drone gone far from the center ($T_{x}$ cm from the origin).
